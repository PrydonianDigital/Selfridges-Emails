...
<xsl:template name="CountryCode">
    ...
    <xsl:choose>
		<xsl:when test="$ccode = 'AC'">Ascension Island</xsl:when>
		...
		<xsl:when test="$ccode = 'AN'">Netherlands Antilles</xsl:when>
		...
		<xsl:when test="$ccode = 'BN'">Brunei Darussalam</xsl:when>
		...
		<xsl:when test="$ccode = 'CD'">Congo, Democratic Republic</xsl:when>
		...
		<xsl:when test="$ccode = 'CI'">Cote D'Ivoire</xsl:when>
		...
		<xsl:when test="$ccode = 'CS'">Czechoslovakia (former)</xsl:when>
		...
		<xsl:when test="$ccode = 'EU'">European Union</xsl:when>
		...
		<xsl:when test="$ccode = 'FX'">France, Metropolitan</xsl:when>
		<xsl:when test="$ccode = 'GA'">Gabon</xsl:when>
		<xsl:when test="$ccode = 'GB'">Great Britain</xsl:when>
		...
		<xsl:when test="$ccode = 'GG'">Guernsey</xsl:when>
		...
		<xsl:when test="$ccode = 'GS'">S. Georgia and S. Sandwich Isls.</xsl:when>
		...
		<xsl:when test="$ccode = 'HM'">Heard and McDonald Islands</xsl:when>
		<xsl:when test="$ccode = 'HN'">Honduras</xsl:when>
		<xsl:when test="$ccode = 'HR'">Croatia (Hrvatska)</xsl:when>
		...
		<xsl:when test="$ccode = 'IM'">Isle of Man</xsl:when>
		...
		<xsl:when test="$ccode = 'JE'">Jersey</xsl:when>
		...
		<xsl:when test="$ccode = 'KP'">Korea (North)</xsl:when>
		<xsl:when test="$ccode = 'KR'">Korea (South)</xsl:when>
		...
		<xsl:when test="$ccode = 'MK'">F.Y.R.O.M. (Macedonia)</xsl:when>
		...
		<xsl:when test="$ccode = 'NT'">Neutral Zone</xsl:when>
		<xsl:when test="$ccode = 'NU'">Niue</xsl:when>
		<xsl:when test="$ccode = 'NZ'">New Zealand (Aotearoa)</xsl:when>
		...
		<xsl:when test="$ccode = 'PM'">St. Pierre and Miquelon</xsl:when>
		<xsl:when test="$ccode = 'PN'">Pitcairn</xsl:when>
		<xsl:when test="$ccode = 'PR'">Puerto Rico</xsl:when>
		<xsl:when test="$ccode = 'PS'">Palestinian Territory, Occupied</xsl:when>
		...
		<xsl:when test="$ccode = 'SH'">St. Helena</xsl:when>
		<xsl:when test="$ccode = 'SI'">Slovenia</xsl:when>
		<xsl:when test="$ccode = 'SJ'">Svalbard &amp; Jan Mayen Islands</xsl:when>
		<xsl:when test="$ccode = 'SK'">Slovak Republic</xsl:when>
		...
		<xsl:when test="$ccode = 'SU'">USSR (former)</xsl:when>
		...
		<xsl:when test="$ccode = 'TP'">East Timor</xsl:when>
		...
		<xsl:when test="$ccode = 'UK'">United Kingdom</xsl:when>
		<xsl:when test="$ccode = 'UM'">US Minor Outlying Islands</xsl:when>
		...
		<xsl:when test="$ccode = 'VA'">Vatican City State (Holy See)</xsl:when>
		<xsl:when test="$ccode = 'VC'">Saint Vincent &amp; the Grenadines</xsl:when>
		<xsl:when test="$ccode = 'VE'">Venezuela</xsl:when>
		<xsl:when test="$ccode = 'VG'">British Virgin Islands</xsl:when>
		<xsl:when test="$ccode = 'VI'">Virgin Islands (U.S.)</xsl:when>
		<xsl:when test="$ccode = 'VN'">Viet Nam</xsl:when>
		<xsl:when test="$ccode = 'VU'">Vanuatu</xsl:when>
		<xsl:when test="$ccode = 'WF'">Wallis and Futuna Islands</xsl:when>
		<xsl:when test="$ccode = 'WS'">Samoa</xsl:when>
		<xsl:when test="$ccode = 'YE'">Yemen</xsl:when>
		<xsl:when test="$ccode = 'YT'">Mayotte</xsl:when>
		<xsl:when test="$ccode = 'YU'">Serbia and Montenegro (former Yugoslavia)</xsl:when>
		<xsl:when test="$ccode = 'ZA'">South Africa</xsl:when>
		<xsl:when test="$ccode = 'ZM'">Zambia</xsl:when>
		<xsl:when test="$ccode = 'ZR'">Zaire</xsl:when>
